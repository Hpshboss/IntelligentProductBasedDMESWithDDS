...
class ProductRepSubscriber
{
    public:

        ProductRepSubscriber(std::shared_ptr<spdlog::logger> logger);

        virtual ~ProductRepSubscriber();

        // initial
        bool init();

        productRep* public_productRep = &m_listener.m_productRep;
